...
export class HttpRequestAnyTool implements INodeType {
	description: INodeTypeDescription;

	constructor() {
		this.description = {
			displayName: 'HTTP Request Any Tool',
			name: 'httpRequestAnyTool',
			icon: 'fa:globe',
			group: ['transform'],
			description: 'Makes an HTTP request and allows custom error handling',
			subtitle: '={{$parameter["method"] + ": " + $parameter["url"]}}',
			version: 1,
			defaults: {
				name: 'HTTP Request Any Tool',
				color: '#0004F5',
			},
			...
			try {
				...
				if (errorItems[itemIndex]) {
					if (onError === 'customJson') {
						let customJson = {};
						try {
							customJson = jsonParse(customErrorJson || '{}');
						} catch (e) {}
						returnItems.push({
							json: customJson as IDataObject,
